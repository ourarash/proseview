---
name: review-prosview-code
description: Conduct a skeptical, read-only senior code and architecture review of Prosview changes or the current implementation, using current repository discovery, a specification-compliance gate, and detailed evidence lanes for domain fidelity, architecture, interfaces, security, reliability, UI/accessibility, performance, tests, packaging, and operations. Use when Codex is asked to review a Prosview diff, pull request, feature, fix, refactor, implementation produced by $develop-prosview, remediation, or repository architecture; identify must-fix issues; validate a correction; or issue an approval verdict with a rigorous self-check.
---

# Review Prosview Code
...
- Read `../develop-prosview/references/prosview-architecture.md` for durable
  Prosview invariants and verify current ownership from the profile.
...
2. Identify affected persistent/public contracts and high-risk Prosview
...
1. A current Prosview contract, invariant, or user/developer workflow.
...
- Recommendations describe the smallest required behavior; implementation
  belongs to `$develop-prosview` or the engineering loop.
